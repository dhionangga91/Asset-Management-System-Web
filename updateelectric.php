...
if(!isset($_SESSION['username'])) {
  ...
  if (isset($_GET['id'])) {
    ...
    $query = "SELECT * FROM electric WHERE id='$id'";
    ...
    header("location:dashboardelectric.php");
  }
  ...
  <div class="nav">
	...
	<div class="userinfo" style="font-family:lato"><strong>Hello !!!</strong> Welcome back <strong>
		...
		<div class="contentcrud">
		     <form id="form_electric" action="../UAS/edit_proseselectric.php" method="post" onSubmit="return validasi()">
			...
			<p><label for="type" style="margin-left: 10px;">Type </label>
			    ...
			    <option value="Breaker" <?php if($data['type'] == 'Breaker'){ echo 'selected'; } ?>>
                Breaker </option>
                <option value="Sensor" <?php if($data['type'] == 'Sensor'){ echo 'selected'; } ?>>
                Sensor</option>
                <option value="Distribution-Board" <?php if($data['type'] == 'Distribution-Board'){ echo 'selected'; } ?>>
                Distribution-Board</option>
	            <option value="Main-Board" <?php if($data['type'] == 'Main-Board'){ echo 'selected'; } ?>>
                Main-Board</option>
                            </select>
				</p>
	  </fieldset>
